 ...
 * We return with mmap_sem still held, but pte unmapped and unlocked.
 */
static vm_fault_t do_anonymous_page_original(struct vm_fault *vmf)
{
	struct vm_area_struct *vma = vmf->vma;
	struct mem_cgroup *memcg;
	struct page *page;
...
/*
 * OS HW2:
 * Helper function to do_anonymous_page() to allocate another page
 * at the specified address.
 */
static int extra_anon_pte_alloc(struct vm_fault *vmf, unsigned long new_addr)
{
	/* Extra variables for allocating extra pages on fault */
	struct vm_area_struct *vma = vmf->vma;
	struct mm_struct *mm = vma->vm_mm;
	...
	/* Determine the pmd of the extra page */
	pgd2 = pgd_offset(mm, new_addr);
	if (pgd_none(*pgd2) || unlikely(pgd_bad(*pgd2)))
		return -1;

	p4d2 = p4d_offset(pgd2, new_addr);
	if (p4d_none(*p4d2) || unlikely(p4d_bad(*p4d2)))
		return -1;

	pud2 = pud_offset(p4d2, new_addr);
	if (pud_none(*pud2) || unlikely(pud_bad(*pud2)))
		return -1;
	
	pmd2 = pmd_offset(pud2, new_addr);
	...
	vmf2.gfp_mask = vmf->gfp_mask;
	vmf2.pgoff = vmf->pgoff;
	vmf2.cow_page = vmf->cow_page;
	vmf2.memcg = vmf->memcg;

	vmf2.address = new_addr;
	vmf2.pmd = pmd2;
	vmf2.pud = pud2;

	/* Check that the new page is not already allocated */
	vmf2.pte = pte_offset_map(vmf2.pmd, vmf2.address);
	vmf2.orig_pte = *(vmf2.pte);
	barrier();
	if (pte_none(vmf2.orig_pte)) {
		pte_unmap(vmf2.pte);
		vmf2.pte = NULL;
	}
	else {
	...
	/* Use existing function to allocate another page */
	do_anonymous_page_original(&vmf2);

	/* Do the remaining work from the caller function, handle_pte_fault */
	vmf2.ptl = pte_lockptr(vmf2.vma->vm_mm, vmf2.pmd);
	spin_lock(vmf2.ptl);
	entry = vmf2.orig_pte;
	if (unlikely(!pte_same(*vmf2.pte, entry)))
		goto unlock;
	if (vmf2.flags & FAULT_FLAG_WRITE) {
		if (!pte_write(entry)){
			do_wp_page(&vmf2);
			return 0;
		}
	...
	entry = pte_mkyoung(entry);
	if (ptep_set_access_flags(vmf2.vma, vmf2.address, vmf2.pte, entry,
				vmf2.flags & FAULT_FLAG_WRITE)) {
		update_mmu_cache(vmf2.vma, vmf2.address, vmf2.pte);
	} else {
		/*
		 ...
		 */
		if (vmf2.flags & FAULT_FLAG_WRITE)
			flush_tlb_fix_spurious_fault(vmf2.vma, vmf2.address);
	}
unlock:
...
/*
 * OS HW2:
 * Revised the do_anonymous page to call the original version for
 * the pte where the fault occured, as well as the next pte, if
 * applicable.
 */
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	/*
	 * First perform the original function and obtain the correct
	 * return value.
	 */
	struct mm_struct *mm = vmf->vma->vm_mm;
	vm_fault_t ret = do_anonymous_page_original(vmf);
	int num_allocs = mm->multi_alloc;
	int i, j;

	if(num_allocs < 1 || num_allocs > 32){
		num_allocs = 1;
		mm->multi_alloc = 1;
	}
	...
	for(i = 1; i < num_allocs; i++){
		j = extra_anon_pte_alloc(vmf, vmf->address + i * PAGE_SIZE);
		if(j < 0)
			break;
	}
