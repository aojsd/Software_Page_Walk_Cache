...
/*
 * OS HW 2 Blank System Call
 * Performs a dummy system call to measure syscall overhead
 */
...
/*
 * OS HW 2 Page Multi Alloc System Call
 * Sets the page multi allocation parameter for the calling
 * process.
 */
SYSCALL_DEFINE1(page_multi_alloc, int, alloc_num)
{
    struct mm_struct *mm = current->mm;

    if(alloc_num < 1 || alloc_num > 32){
        return -1;
    }

    if(mm){
        mm->multi_alloc = alloc_num;
    }
...
/*
 * OS Semester Project MM Parameter Reset System Call
 * Resets the timers and multi alloc parameters in the mm_struct
 */
SYSCALL_DEFINE0(mm_param_reset)
{
	struct mm_struct *mm = current->mm;
	mm->multi_alloc = 1;
	mm->page_walk_time = 0;
